...
public class PersistentCookieStore {
    ...
    public PersistentCookieStore(Context context) {
        ...
        for (Map.Entry<String, ?> entry : prefsMap.entrySet()) {
            Log.d(TAG, "PersistentCookieStore 1: [ entry.getKey()：" + entry.getKey() + ",entry.getValues：" + entry.getValue() + "]");
            if (null != entry.getValue() && !((String) entry.getValue()).startsWith(COOKIE_NAME_PREFIX)) {
                ...
                for (String name : cookieNames) {
                    ...
                    Log.d(TAG, "PersistentCookieStore 2: [ name：" + name + ",encodedCookie：" +encodedCookie + "]");
                    if (encodedCookie != null) {
                        Cookie decodedCookie = decodeCookie(encodedCookie);
                        Log.d(TAG, "PersistentCookieStore 3: [ name：" + name + ",decodeCookie：" + decodedCookie.toString() + "]");
...
//        String name = getCookieToken(cookie);
        ...
        Log.d(TAG, "PersistentCookieStore add: [ keySet：" + mCookies.get(cookie.domain()).keySet() + "]");
        Log.d(TAG, "PersistentCookieStore add: [ cookie.name()：" + cookie.toString() + "]");
